...
const Security: React.FC = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await fetch(`/api/users?clinic=${encodeURIComponent(user.clinic)}`, {
        credentials: 'include'
      });
      ...
      // Map clinic_id to clinic name
      const clinicIdToName: { [key: number]: string } = {
        1: 'Universal Hospital'
      };
      ...
      const transformedUsers = usersData.map((u: any) => ({
        ...
        email: u.email || '',
        role: u.role || '',
        clinic: clinicIdToName[u.clinic_id] || u.clinic || '',
        isActive: u.is_active !== undefined ? u.is_active : true,
        createdAt: u.created_at || u.createdAt,
        lastLogin: u.last_login || u.lastLogin
      })) as UserWithStatus[];
      
      setUsers(transformedUsers.sort((a, b) => a.displayName.localeCompare(b.displayName)));
    } catch (error) {
  ...
  const handleEditUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`User ${editFormData.displayName || 'User'} updated successfully`);
